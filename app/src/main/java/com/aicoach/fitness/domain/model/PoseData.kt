...
fun NormalizedLandmark.toBodyLandmark(name: String): BodyLandmark {
    return BodyLandmark(
        name = name,
        x = this.x(),
        y = this.y(),
        z = this.z(),
        visibility = this.visibility().orElse(0f),
        presence = this.presence().orElse(0f)
    )
}

fun Landmark.toBodyLandmark(name: String): BodyLandmark {
    return BodyLandmark(
        name = name,
        x = this.x(),
        y = this.y(),
